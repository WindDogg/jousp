...
public class ZhiLianZhaoPin {
	...
	private final static String url_pre = "https://fe-api.zhaopin.com/c/i/jobs/keyword-company-search?at=03be05e5278c4440b2c7cbc552ac8355&rt=de488d5d416a426e8a553f58b30b12fd&keyword=";
	private final static String url_after = "&filter_c_workCity=530&_v=0.66023375&x-zp-page-request-id=83f4caad17c341e8b8714f7d38609c1b-1610442046124-624895&x-zp-client-id=b7e449cf-5e30-4044-828f-528d69bc9745";
	...
	public void go() throws InterruptedException, IOException {
		...
		List<Map<String, Object>> onameList = MysqlHelper.executeQuery("SELECT oname from company_net_zhaopin where state is null");
		
		Document doc = null;
		Random random = new Random();
		
		for(int i = 0; i<onameList.size(); i++){
			String oname = StringUtils.trimToEmpty((String)onameList.get(i).get("oname"));
...
//					.header("User-Agent", "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:84.0) Gecko/20100101 Firefox/84.0")
			...
			JSONObject dataJson = bdataJson.getJSONObject("data");
			if(dataJson==null){
				MysqlHelper.executeUpdate("update company_net_zhaopin set zpid = ?, state = '0' where type='zhilian' and oname=?",new String[]{"",oname});
				continue;
			}
			String companyNumber = StringUtils.trimToEmpty(dataJson.getString("companyNumber"));
			
			if(!"".equals(companyNumber)){
				MysqlHelper.executeUpdate("update company_net_zhaopin set zpid = ?, state = '1' where type='zhilian' and oname=?",new String[]{companyNumber,oname});
			}else{
				MysqlHelper.executeUpdate("update company_net_zhaopin set zpid = ?, state = '0' where type='zhilian' and oname=?",new String[]{"",oname});
			}
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		ZhiLianZhaoPin obj = new ZhiLianZhaoPin();
		obj.go();
