...
<%
	...
	try {
		...
		qyList = dbclass.doQuery(conn, "select id,bdshape from xxandyy t where bdshape <> ''");
		if (qyList == null) {
			out.println(dbclass.getMsg());
		}
	} catch (Exception ex) {
		...
		$.get('../json/hdjd_new_baidu.json', function(data) {
			...
			$.each(polys, function(i, obj) {
				...
				$.each(qyList, function(j, qyo) {
					var bdshape = qyo.bdshape;
					var shapes = bdshape.split(';');
					var lnglat = shapes[0].split(',');
					var p = new BMapGL.Point(lnglat[0], lnglat[1]);
